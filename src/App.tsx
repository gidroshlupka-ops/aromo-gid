...
const FastScrollHandle = () => {
  ...
  const updateScroll = (clientY: number) => {
    ...
    window.scrollTo({ top: progress * totalHeight, behavior: 'auto' });
  };
  ...
  return (
    <div
      ref={containerRef}
      // CHANGE 1: top-28 вместо top-20 — полоска не доходит до бургера
      className="fixed right-0 top-28 bottom-20 w-12 z-[100] md:hidden flex flex-col items-center touch-none select-none"
    ...
    >
      <div className="relative w-full h-full flex justify-center pointer-events-none">
        ...
        <motion.div
          ...
          style={{ top: `${scrollProgress * 100}%`, translateY: '-50%' }}
          animate={{
...
const Navbar = () => {
  ...
  const menuItems = [
    ...
    { label: 'заказ и доставка', id: 'delivery' },
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 backdrop-blur-lg shadow-sm py-2' : 'bg-transparent py-4'}`}>
      ...
      {/* CHANGE 2: Redesigned mobile menu */}
      <AnimatePresence>
        {isMenuOpen && (
          <>
            <motion.div
            ...
            <motion.div
              ...
              className="fixed top-4 left-4 right-4 bg-white z-[70] shadow-2xl rounded-3xl overflow-hidden md:hidden border border-stone-100"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="flex justify-between items-center px-6 pt-5 pb-4 border-b border-stone-100">
                <div className="flex items-center gap-2.5">
                  <img src={`${import.meta.env.BASE_URL}logo.jpg`} alt="Logo" className="h-8 w-8 rounded-full object-cover border border-stone-200" referrerPolicy="no-referrer" />
                  <span className="font-serif font-bold tracking-widest text-stone-800 text-sm">АРОМА ГИД</span>
                </div>
                <button
                  onClick={() => setIsMenuOpen(false)}
              ...
              <div className="px-4 py-4 flex flex-col gap-2">
                {menuItems.map((item, i) => (
                  <motion.button
                    ...
                    onClick={() => scrollTo(item.id)}
                    className="group relative flex items-center justify-between w-full text-left px-5 py-4 rounded-2xl bg-stone-50 hover:bg-amber-50 border border-transparent hover:border-amber-200 transition-all duration-200"
                  >
                    <span className="uppercase text-xs tracking-[0.2em] font-black text-stone-700 group-hover:text-amber-700 transition-colors">
                      {item.label}
                    </span>
                    <ChevronRight size={14} className="text-stone-300 group-hover:text-amber-400 group-hover:translate-x-0.5 transition-all" />
                  </motion.button>
                ))}
              </div>

              {/* Footer accent */}
              <div className="mx-4 mb-4 mt-1 h-1 rounded-full bg-gradient-to-r from-amber-200 via-amber-400 to-amber-200 opacity-60" />
            </motion.div>
          </>
        )}
      </AnimatePresence>
...
const Hero = () => {
  return (
    // CHANGE 3: removed blur/dim overlay, added gradient fade to white at bottom
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-16 md:mb-20">
        {features.map((f, i) => (
          ...
          >
            ...
            <p className="text-stone-800 text-xl md:text-xl lg:text-xl leading-relaxed font-bold">{f.text}</p>
          </motion.div>
        ))}
      </div>
          ...
          {PERFUMES.map((p) => (
            ...
            >
              <div
                ...
                onClick={() => {
                  setSelectedId(p.id);
                  // Отправляем данные в Метрику о просмотре конкретного аромата
                  if (typeof ym !== 'undefined') {
                    ym(107014178, 'params', {
                      top_perfumes: {
                        opened: p.name // Берем имя из объекта p
                      }
                    });
                  }
                }}
              ...
              <div className="px-1 md:px-2 flex-grow flex flex-col text-left">
                ...
                <div className="mb-2 md:mb-4">
                  <p className="text-stone-500 text-sm md:text-xl truncate font-medium">{p.description}</p>
                </div>
...
const Reviews = () => {
  ...
  const nextFullscreen = (e?: React.MouseEvent) => {
    e?.stopPropagation();
    if (fullscreenIndex !== null) setFullscreenIndex((fullscreenIndex + 1) % REVIEWS.length);
  };

  const prevFullscreen = (e?: React.MouseEvent) => {
    e?.stopPropagation();
    if (fullscreenIndex !== null) setFullscreenIndex((fullscreenIndex - 1 + REVIEWS.length) % REVIEWS.length);
  };
  ...
  const handlePinchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    pinchState.current = { initialDistance: distance, initialScale: zoomScale };
  };
  ...
  const handlePinchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    let nextScale = (distance / pinchState.current.initialDistance) * pinchState.current.initialScale;
  ...
  const handlePinchEnd = () => {
    pinchState.current = null;
    if (zoomScale <= 1.01) { setZoomScale(1); setIsZoomed(false); }
  };
    ...
    <section id="reviews" className="py-16 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 text-center">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          <button onClick={prevReview} className="absolute left-[-10px] md:left-[-30px] top-1/2 -translate-y-1/2 p-2 bg-white/80 rounded-full shadow-lg text-stone-800 hover:bg-white transition-colors z-10">
            <ChevronLeft size={20} />
          </button>
          <button onClick={nextReview} className="absolute right-[-10px] md:right-[-30px] top-1/2 -translate-y-1/2 p-2 bg-white/80 rounded-full shadow-lg text-stone-800 hover:bg-white transition-colors z-10">
      ...
      <AnimatePresence>
        {fullscreenIndex !== null && (
          <motion.div
            ...
            onClick={() => { setFullscreenIndex(null); setZoomScale(1); setIsZoomed(false); }}
          >
            ...
            <button onClick={prevFullscreen} className="absolute left-4 md:left-10 top-1/2 -translate-y-1/2 p-4 text-white/50 hover:text-white transition-colors z-[120] hidden md:block">
              <ChevronLeft size={64} strokeWidth={1} />
            </button>
            <button onClick={nextFullscreen} className="absolute right-4 md:right-10 top-1/2 -translate-y-1/2 p-4 text-white/50 hover:text-white transition-colors z-[120] hidden md:block">
              <ChevronRight size={64} strokeWidth={1} />
            </button>
                ...
                >
                  ...
                  >
                    <motion.img
                      ...
                      animate={{ scale: zoomScale, x: isZoomed ? undefined : 0, y: isZoomed ? undefined : 0 }}
                      drag={isZoomed}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div className="space-y-8">
            <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left p-6 glass-card rounded-2xl">
              ...
              <div>
                ...
                <a href="tel:+79999999999" className="text-xl md:text-2xl font-black text-stone-800 hover:text-emerald-600 transition-colors" onClick={() => typeof ym !== 'undefined' && ym(107014178, 'reachGoal', 'call_main')}>+7 (999) 999-99-9                                                                                      9</a>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left p-6 glass-card rounded-2xl">
              ...
              <div className="flex flex-col items-center md:items-start">
                ...
                <a href="https://t.me" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 bg-blue-50 text-blue-600 hover:bg-blue-100 px-6 py-3 rounded-full transition-colors font-black text-lg" onClick={() => typeof ym !== 'undefined' && ym(107014178, 'reachGoal', 'tg_personal')}>
                  <span>@Telegram</span>
                </a>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left p-6 glass-card rounded-2xl">
              ...
              <div>
                ...
                <a href="tel:+79999999999" className="text-xl md:text-2xl font-black text-stone-800 hover:text-stone-600 transition-colors">+7 (999) 999-99-99</a>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div className="p-8 glass-card rounded-3xl border-amber-100">
              ...
              <ul className="space-y-6 text-base md:text-lg text-stone-600 text-center lg:text-left font-medium">
                <li>
                  ...
                  <a href="https://t.me" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all font-bold" onClick={() => typeof ym !== 'undefined' && ym(107014178, 'reachGoal', 'tg_bot')}>aromo</a>
                </li>
                <li>
                  <p className="font-bold mb-2 text-stone-800">Наш чат: отвечаем на ваши вопросы и рассказываем об интересном</p>
                  <a href="https://t.me" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all font-bold" onClick={() => typeof ym !== 'undefined' && ym(107014178, 'reachGoal', 'tg_chat')}>aromochat</a>
                </li>
              </ul>
            </div>
            ...
            <div className="p-8 glass-card rounded-3xl text-center lg:text-left">
              ...
              <a href="https://t.me" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 bg-blue-50 text-blue-600 hover:bg-blue-100 px-6 py-3 rounded-full transition-colors font-black text-lg" onClick={() => typeof ym !== 'undefined' && ym(107014178, 'reachGoal', 'tg_partner')}>
                <Send size={20} />
                <span>@Telegram</span>
              </a>
            </div>
